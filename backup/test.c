...
void estimateCron(int month, char *cronFile, char *estFile)
{
    ...
    for (int i = 0; i < linesAmt; i++)
    {
        ...
        printf("%s %d\n", commands[i].command, commands[i].duration);
    }
    printf("\n------------------------------------\n");
    ...
    while (1)
    {
        ...
        for (int i = 0; i < linesAmt; i++)
        {
                ...
                (weekday == commands[i].dayOfWeek || commands[i].dayOfWeek == -1))
            {
                ...
                commands[i].counter++;
                // printf("@ %02d:%02d\t %02d/%02d invoke (pid=%d, running=%d) duration=%d\t %d %d %d %d %d\t %s\n", hour, minute, date, month, amtProcesses, queue, status[amtProcesses].remainingTime, commands[i].min, commands[i].hr, commands[i].day, commands[i].month, commands[i].dayOfWeek, status[amtProcesses].command);
                amtProcesses++;
            }
        }
        if (queue > maxQueue)
                 ...
                 (weekday == commands[i].dayOfWeek || commands[i].dayOfWeek == -1))
        {
            ...
            strcpy(status[amtProcesses].command, commands[i].command);
            status[amtProcesses].state = true;
            status[amtProcesses].remainingTime = commands[i].duration - 1;
            commands[i].state = true;
            commands[i].remainingTime = commands[i].duration - 1;
            amtProcesses++;
            commands[i].counter++;
            printf("@ %02d:%02d\t %02d/%02d invoke (pid=%d, running=%d)\t %d %d %d %d %d\t %s\n", hour, minute, date, month, amtProcesses, queue, commands[i].min, commands[i].hr, commands[i].day, commands[i].month, commands[i].dayOfWeek, commands[i].command);
        }
        ...
        for (int j = 0; j < sizeof(status->command); j++)
        {
            if (status[j].state == true &&
                status[j].remainingTime > 0)
